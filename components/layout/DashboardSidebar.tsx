...
export default function DashboardSidebar({ isOpen, onClose }: DashboardSidebarProps) {
  ...
  const homeHref = user?.role === 'salesman' ? '/dashboard/taking-order' : '/dashboard';
  ...
  const sidebarContent = (
    <div className="h-full w-full max-w-full flex flex-col justify-between bg-white text-slate-800 overflow-hidden">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="lg:hidden p-1.5 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition cursor-pointer"
        ...
        {/* System Sub-badge with User Territory */}
        <div className="mt-3.5 flex items-center justify-between px-2.5 py-1.5 bg-blue-50/70 border border-blue-100 rounded-xl text-[11px] text-blue-700 font-semibold">
          <span className="flex items-center gap-1.5 truncate">
            ...
            <span className="truncate">
              {user?.role === 'supervisor' ? `SPV: ${user.area}` : `Sales: ${user?.area || 'Bandung'}`}
            </span>
          </span>
          <span className="text-[10px] bg-blue-200/70 px-1.5 py-0.5 rounded text-blue-800 font-bold shrink-0">
            {user?.role === 'supervisor' ? 'Supervisi' : 'SFA'}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-3.5 py-4 space-y-6">
        {/* Group 1: Supervisi & Konsolidasi (Shown ONLY for Supervisor) */}
          ...
          <div>
            <div className="px-2 mb-2.5 flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-800 bg-indigo-50/90 px-2 py-1 rounded-lg border border-indigo-100 flex items-center gap-1.5">
                ...
                Role: Supervisor ({user?.area || 'Wilayah'})
              </span>
            </div>
        ...
        {/* Group 2: Operasional Sales Lapangan (Shown ONLY for Salesman) */}
        {user?.role === 'salesman' && (
          <div>
            <div className="px-2 mb-2.5 flex items-center justify-between">
